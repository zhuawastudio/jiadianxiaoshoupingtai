        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jiadianCommentback','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jiadianCommentback.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('jiadianCommentback','查看')"
                          :data="dataList"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jiadianCommentback','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jiadianCommentback','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('jiadianCommentback','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jiadian_commentback',//当前表表名,
                        sumColum : 'jiadian_commentback_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jiadian_commentback_name',//分组字符串字段
                        // types : 'jiadian_commentback_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //     var statistic = this.$echarts.init(document.getElementById("statistic"),'macarons');
                //     let params = {
                //         tableName: "jiadian_commentback",
                //         groupColumn: "jiadian_commentback_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['jiadianCommentbackDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            jiadianCommentbackUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jiadianCommentback/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            jiadianCommentbackUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "jiadianCommentback/update",
                        method: "post",
                        data: {
                            id:id,
//                            jiadianCommentbackTypes:jiadianCommentbackTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
