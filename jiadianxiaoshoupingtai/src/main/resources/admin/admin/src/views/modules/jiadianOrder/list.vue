        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '订单号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.jiadianOrderUuidNumber" placeholder="订单号" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jiadianOrder','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jiadianxiaoshoupingtai/upload/jiadianOrderMuBan.xls"
                        >批量导入商品订单数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jiadianOrder','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jiadianOrderUploadSuccess"
                                :on-error="jiadianOrderUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jiadianOrder','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jiadianOrder.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="jiadianPhoto"
                               header-align="center"
                               width="200"
                               label="商品照片">
                        <template slot-scope="scope">
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jiadianOrderPaymentTypes"
                                      header-align="center"
                                      label="支付类型">
                        <template slot-scope="scope">
                            {{scope.row.jiadianOrderPaymentValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jiadianOrder','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jiadianOrder','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('jiadianOrder','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
            <el-button v-if="isAuth('jiadianOrder','订单') && scope.row.jiadianOrderTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">退款</el-button>
            <el-button v-if="isAuth('jiadianOrder','订单') && scope.row.jiadianOrderTypes == 104 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="openCommentModel(scope.row.id)">评价</el-button>
            <el-button v-if="isAuth('jiadianOrder','订单') && scope.row.jiadianOrderTypes == 101
&& sessionTable=='users'" type="primary" icon="el-icon-reading" size="mini"
                        @click="openDeliver(scope.row.id)"
            >发货</el-button>
            <el-button v-if="isAuth('jiadianOrder','订单') && scope.row.jiadianOrderTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="receiving(scope.row.id)">收货</el-button>
...
<script>
    ...
    export default {
        ...
        return {
            ...
            deliverId:null,
			//快递单号
            jiadianOrderCourierNumber:null,
			//快递公司
            jiadianOrderCourierName:null,
			//模态框状态
    ...
    //当前表下拉框搜索条件
              jiadianOrderTypesSelectSearch : [],
            form:{
                id : null,
                jiadianOrderUuidNumber : null,
                addressId : null,
                jiadianId : null,
                yonghuId : null,
                buyNumber : null,
                jiadianOrderTruePrice : null,
                jiadianOrderCourierName : null,
                jiadianOrderCourierNumber : null,
                jiadianOrderTypes : null,
                jiadianOrderPaymentTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '商品名称': 'jiadianName',
                     '商品编号': 'jiadianUuidNumber',
                     '商品照片': 'jiadianPhoto',
                     '商品类型': 'jiadianTypes',
                     '商品库存': 'jiadianKucunNumber',
                     '商品原价': 'jiadianOldMoney',
                     '现价': 'jiadianNewMoney',
                     '商品热度': 'jiadianClicknum',
                     '是否上架': 'shangxiaTypes',
                     '用户姓名': 'yonghuName',
                     '联系方式': 'yonghuPhone',
                     '身份证号': 'yonghuIdNumber',
                     ...
                     '余额': 'newMoney',
                //本表字段
                     '订单号': "jiadianOrderUuidNumber",
                     '购买数量': "buyNumber",
                     '实付价格': "jiadianOrderTruePrice",
                     '快递公司': "jiadianOrderCourierName",
                     '订单快递单号': "jiadianOrderCourierNumber",
                     '订单类型': "jiadianOrderValue",
                     '支付类型': "jiadianOrderPaymentValue",
                     '订单创建时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jiadian_order',//当前表表名,
                        sumColum : 'jiadian_order_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jiadian_order_name',//分组字符串字段
                        // types : 'jiadian_order_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jiadian_order_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.jiadianOrderUuidNumber!= '' && this.searchForm.jiadianOrderUuidNumber!= undefined) {
                    params['jiadianOrderUuidNumber'] = '%' + this.searchForm.jiadianOrderUuidNumber + '%'
                }
                                                                 
                if (this.searchForm.jiadianOrderTypes!= '' && this.searchForm.jiadianOrderTypes!= undefined) {
                    params['jiadianOrderTypes'] = this.searchForm.jiadianOrderTypes
                }
                        
                params['jiadianOrderDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            jiadianOrderUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jiadianOrder/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
			...
			deliver(){
				...
				_this.$confirm(`确定要发货吗?`, "提示", {
					...
					_this.$http({
						url: "jiadianOrder/deliver?id="+id+"&jiadianOrderCourierNumber="+_this.jiadianOrderCourierNumber+"&jiadianOrderCourierName="+_this.jiadianOrderCourierName,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
			...
			receiving(id){
				this.$confirm(`确定要收货吗?`, "提示", {
					...
					this.$http({
						url: "jiadianOrder/receiving?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
			...
			commentback(id){
				...
				_this.$confirm(`确定要评价吗?`, "提示", {
					...
					_this.$http({
						url: "jiadianOrder/commentback?id="+_this.commentbackId+"&commentbackText="+_this.commentbackContent+"&jiadianCommentbackPingfenNumber="+_this.jiadianCommentbackPingfenNumber,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
								...
								onClose: () => {
									...
									//情况评分
									_this.jiadianCommentbackPingfenNumber = null
								}
							});
						} else {
							//清空操作数据id
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "jiadianOrder/update",
                        method: "post",
                        data: {
                            id:id,
//                            jiadianOrderTypes:jiadianOrderTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
